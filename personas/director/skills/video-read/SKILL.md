...
영상은 "읽을" 수 없습니다. **화면은 그림으로 만들어 눈으로 보고**, **말은 받아 적어서
읽습니다.** 아래 상황에 맞는 것을 씁니다. 어느 쪽도 안 되면 안 됐다고 말하세요 — 제목만
보고 내용을 지어내면 안 됩니다.
...
- 받아쓰기: `transcribe.mjs` (작업 폴더에 그냥 놓여 있습니다)

(Windows가 아니면 `.exe` 없이 같은 경로입니다.)

## 말을 듣는 법 — `transcribe.mjs`

```bash
node transcribe.mjs "<영상 또는 소리 파일>" --lang ko --out 말.txt
```

- `--lang` 은 `ko` / `en` / `auto`. 무슨 말인지 알면 지정하는 쪽이 정확합니다.
- 결과는 `[00:12] 말한 내용` 꼴로 시간과 함께 나옵니다. **시간대별 흐름은 이걸로 만드세요.**
- 영상 파일을 그대로 넣으면 됩니다 — 소리는 알아서 꺼냅니다.
- **처음 한 번은 모델을 받느라 몇 분 걸립니다.** 그때는 사용자에게 "말을 받아 적을 준비를
  하고 있어요, 처음 한 번만 걸립니다" 라고 한 줄 말해 두세요. 그 다음부터는 30초 소리에
  20초쯤입니다.
- 소리가 없는 영상이면 그렇게 알려주고 끝냅니다 — 화면만 보고 말하면 됩니다.

받아쓴 것은 **기계가 들은 것**입니다. 사람 이름·제품명·숫자는 틀리기 쉬우니, 중요한 대목은
화면에 뜬 글자와 맞춰 보세요.
...
**4) 말은 받아 적습니다.** 위의 `transcribe.mjs` 를 같은 파일에 그대로 돌리면 됩니다.
화면과 말을 시간으로 맞춰 붙이면 그게 이 영상의 내용입니다.
...
### B-1.5. 자막이 없으면 소리를 받아 옵니다

**영상은 못 받아도 소리는 받아지는 경우가 많습니다** (유튜브가 그렇습니다 — 화면 쪽 형식만
막혀 있고 소리 형식은 열려 있습니다).

```bash
node_modules/youtube-dl-exec/bin/yt-dlp.exe \
  --js-runtimes node --ffmpeg-location "node_modules/ffmpeg-static/ffmpeg.exe" \
  -f "bestaudio/best" -o "audio.%(ext)s" "<URL>"

node transcribe.mjs audio.webm --lang ko --out 말.txt
```

이게 되면 자막이 없는 영상도 말한 내용을 시간과 함께 알 수 있습니다. 소리마저 못 받으면
(틱톡이 그렇습니다) 화면만 보고, **말한 내용은 모른다**고 밝히세요.
...
- **자막도 소리도 화면도 못 얻었으면** 요약하지 마세요. 무엇이 막혔는지 한 줄로 말하고
  끝냅니다.
...
마지막에 **어디까지 직접 봤고 들었는지** 한 줄 붙입니다.
예: "화면은 4장(2·7·11·16초)을 봤고, 말은 받아쓰기 기준입니다 — 제품명은 화면 글자와
맞췄습니다."
